...
- Control: Paz mental para el dueño, sabiendo exactamente qué sucede en su negocio en tiempo real.
...
| :--- | :---: | ---: | ---: |
| App Nativa | Alto rendimiento, acceso total al hardware | Costo de desarrollo alto, fricción de descargar | Descartada: los costos de desarrollo no son asumibkes ni por el desarrollador ni los negocios.  |
| Web Tradicional | Acceso universal, sin instalación. | Dependencia total de internet | Aceptada como primer MVP: debdio a el manejo de las tecnologías de desarrollo sera lo más viable, además se trata de búscar interesados en usar la herramienta, por defecto no se necesita algo exesibamente grande que genere cosaotos adicionles injustificados |
| PWA | Instalación inmediata, offline parcial, ligera y multiplataforma | Acceso limitado a hardware avanzado | Elegida a largo plazo: Combina la facilidad de una web con la presencia de una app. Es la solución más realista para el hardware de gama media local, solo se acoplara si la web lo necesita a nivel negocio |
...
- Criterio: Debe registrar: Variante vendida + Precio + Método de pago  + checar si es fiado.
...
- Logica: Al ser un entorno local, el cierre de venta ocurre en el chat de whatsapp. El catálogo genera un mensaje pre-formateado, esto reduce la fricción para el consumidor final.
- Ventaja: El dueño no tiene que crear un catálogo aparte en PDF o Facebook. Al cargar el inventario para su control interno, ya está haciendo su marketing digital sin trabajo extra.
...
- Ejemplo:
    ...
    - Si el cliente no tiene todo el dinero, el Empleado marca el resto como Fiado (Capa 6), vinculando la deuda al perfil del cliente para que el Administrador lo vea en sus reportes.
...
- Se usá un Middleware de Contexto para que siempre se agrege el condicional que identifica a cada negocio.
    - Identificación: KODA detectará quién está pidiendo la información mediante el token de sesión del usuario.
...
- **RF0.1:** El Super-Admin debe poder registrar manualmente un nuevo negocio en la base de datos (Nombre del local, Dueño, Teléfono) y también un negocio se puede registrar desde la landing del SaaS.
...
- **RF0.3:** El sistema debe restringir el acceso a la cuenta si el estado de la suscripción cambia a "Suspendido" (modificable solo por el Super-Admin en caso de algún error del sistema, sino solo se cambiará si se cancela la suscripción).
...
- **RF5.4:** Al confirmar la venta, el sistema debe descontar de manera atómica el stock específico de la variante (talla/color) seleccionada (Capa 3) de manera inmediata.
...
### Capa 1: Categorías
- **Historia:** Como Administrador de negocio, quiero agrupar mis prendas por tipos (ej. Camisetas, Jeans), para que tanto en el catálogo público como en el punto de venta sea más fácil localizar lo que buco.
...
### Capa 4: Empleados
- **Historia:** Como Administrador de negocio, quiero crear perfiles para mis vendedores con un código de acceso simple, para saber quién realizó cada venta/abono y evitar errores en el cierre de caja.
...
**UC1: Alta de Negocio**
- **Actor:** Super-Admin.
- **Descripción:** Registra un local (en caso de ser necesario), se asigna un `tenantId` y se crea el primer usuario Admin. Si el correo del dueño ya existe, el sistema impide el registro duplicado.

**UC2: Suspensión de Servicio**
- **Actor:** Super-Admin.
...
- **UC1: Cambio de Turno**
    ...
    - **Camino Alterno:** Si el PIN es incorrecto 3 veces, el sistema bloquea el acceso por 1 minuto por seguridad y notifica al admin.
...
- **UC2: Aplicación de Descuento**
    - **Actor:** Empleado.
    - **Descripción:** Reducción del precio final por promoción o fidelidad.
    - **Camino Alterno:** El Empleado puede colocar un Descuento por compra.

**Capa 6: Resolución de Fiados**
- **UC1: Apertura de Crédito**
    - **Actor:** Empleado.
...
- **updatedAt:** [TIMESTAMP]
> [!NOTE]
> El slug es vital para generar la URL del catálogo público (Capa 7).
...
- **Consistencia de Datos:** Al estar en un solo proyecto con una sola base de datos, las transacciones son atómicas y simples.

## Arquitectura de Despliegue (Infraestructura)

- **Frontend y Backend (App):** Vercel. Al usar Next.js, las rutas de la API se despliegarán automáticamente como Serverless Functions. Es rápido, global y tiene un despliegue por cada "push" a GitHub.
- **Base de Datos:** Neon (Postgres Serverless). Rápido y confiable para los datos de La Unión.
- **Almacenamiento:** Cloudinary para las fotos de las prendas.
- **Validación:** Zod en los controladores para asegurar la integridad de los datos.

### Flujo de Despliegue
1. **Código:** GitHub (Repositorio único).
2. **CI/CD:** Vercel detecta el cambio -> Corre tests -> Despliega.
3. **Conexión:** La App se conecta a la BD Postgres en Neon mediante una Connection Pool para manejar los picos de tráfico de los negocios.

## Estructura de Carpetas (Arquitectura por Capas)

El código se organizará siguiendo una lógica de **Feature-based Architecture** (Arquitectura basada en funcionalidades). Esto evita que el proyecto se vuelva un desorden.
...
│       ├── errors/       # Clases personalizadas (AppError, BusinessError, etc.)
│       └── utils/        # Gestión de respuestas HTTP y logger
...
1. **Componente React:** El botón "Finalizar Venta" dispara un evento.
2. **Hook (`useSales`):** Llama a un servicio de fetch hacia la ruta `/api/[tenantId]/sales`.
3. **API Route:** Recibe la petición y la envía al `SalesController`.
4. **SalesController:** Valida los datos y los pasa al `SalesService`.
5. **SalesService:** Ejecuta la lógica y pide al `SalesRepository` guardar en la DB.
6. **Respuesta:** Todo regresa en cadena hasta que el componente muestra un mensaje de "Venta Exitosa".

## Manejo de Errores

Para que el usuario siempre sepa qué pasa, se implementa una Gestión de Errores por Capas:
...
Se usarán los `error.tsx` de Next.js para que, si un componente falla, el resto del dashboard siga funcionando. El usuario verá un mensaje amable en lugar de una pantalla blanca.

## El Middleware: El "Cerebro" de la Multi-tenencia

Garantiza que el Negocio A no vea los datos del Negocio B. Para ello, usaremos un Middleware que inyecte el `tenantId` en cada sesión.

**Flujo de Seguridad:**
1. El usuario se loguea.
2. El token de sesión guarda su `tenantId` y su `role`.
3. Cada vez que el Frontend hace una petición a la API (ej: `GET /api/products`), el backend extrae el `tenantId` del token de forma automática.
4. Prisma ejecuta la consulta: `prisma.product.findMany({ where: { tenantId: session.tenantId } })`.

## Implementación de las 7 Capas en el Código

Cada una de las 7 capas residirá en la carpeta `core`. Esto permite que, si en el futuro se cambia Prisma por otra herramienta, solo sea necesario modificar los archivos en `core` y el resto de la aplicación (el frontend) sigue funcionando igual.

## Estrategia de Inmersión (Landing + Dashboard)

- **Landing Page (Pública):** Ubicada en `app/page.tsx`. Contiene: KODA, planes y registro.
- **Dashboard (Privado):** Ubicado en `app/(dashboard)`. Un diseño minimalista centrado en la velocidad operativa.
...
1. **Dueño:** Accede al dashboard y crea una Categoría (Capa 1) para organizar su vitrina.
2. **Dueño:** Registra el Producto (Capa 2) subiendo los datos requeridos.
3. **Dueño:** Define las Variantes (Capa 3) especificando la combinación de Talla, Color y demás datos como el Stock inicial disponible.
4. **Sistema:** Valida que no existan SKUs duplicados y guarda la relación jerárquica en la base de datos (Neon).
5. **Sistema:** Sincroniza automáticamente los datos para que el producto sea visible en el Catálogo Público.
...
1. **Empleado:** Inicia sesión con su contraseña y accede al POS (Punto de Venta).
2. **Empleado:** Selecciona los productos y las variantes específicas (Talla/Color) que el cliente desea llevar.
3. **Sistema:** Verifica en tiempo real que haya Stock (Capa 3) suficiente para completar la operación.
4. **Empleado:** Selecciona el método de pago. Si elige "Fiado" (Capa 6), el sistema le obliga a seleccionar o registrar un Cliente.
5. **Sistema:** Si es fiado, suma el total a la deuda del cliente; si es contado, solo registra el ingreso.
6. **Sistema:** Descuenta las unidades del inventario y genera el recibo digital para compartir por WhatsApp.
...
1. **Usuario:** Busca al cliente por nombre o teléfono en el módulo de Clientes (Capa 6).
2. **Sistema:** Muestra el `totalDebt` actual y el historial de ventas fiadas del cliente.
3. **Usuario:** Registra un nuevo Abono (Payment) ingresando el monto recibido y el método (Efectivo/Transferencia).
4. **Sistema:** Resta automáticamente el monto del abono al saldo pendiente del cliente (`totalDebt`).
5. **Sistema:** Registra la transacción en el historial para auditoría y confirma el nuevo saldo en pantalla.

### Flujo de Consulta y Conversión (Capa 7)
- **Actor:** Cliente Final.
- **Objetivo:** Visualizar el catálogo y generar una intención de compra real.
1. **Cliente:** Escanea el código QR del local o abre el enlace.
2. **Sistema:** Filtra el inventario y muestra solo los productos que tienen Stock activo (Capa 3).
3. **Cliente:** Navega por las Categorías (Capa 1) y selecciona una prenda para ver sus fotos y tallas disponibles.
4. **Cliente:** Hace clic en el botón de WhatsApp de una variante específica.
5. **Sistema:** Redirige a WhatsApp con un mensaje pre-configurado que incluye el nombre del producto, talla, color y precio.
6. **Dueño:** Recibe el mensaje y cierra la venta de forma personalizada.
...
1. **Dueño:** Notifica al SuperAdmin que ha realizado el pago mensual (ej: vía transferencia).
2. **SuperAdmin:** Accede al Dashboard Global, busca el Tenant y registra la transacción.
3. **Sistema:** Actualiza la `endDate` de la Subscription del negocio sumando el periodo correspondiente.
4. **Sistema:** Valida que el status cambie a `ACTIVE` (en caso de haber estado suspendido o en mora).
5. **Dueño:** Recibe una notificación en su dashboard confirmando que su acceso a las 7 capas ha sido renovado.
...
- **Resumen:** Aquí aterrizamos las 7 Capas. El problema no es solo vender, es la falta de visibilidad del inventario y el desorden en el recaudo de deudas. Definimos que KODA debe ser un SaaS multi-tenancy con un backend robusto y una UI minimalista, que además busca familiarizar a los negocios con el software mediante el acompañamiento.
...
### Prototipar
- **Funcionalidad Crítica:** El flujo de "Venta -> Descuento de Stock -> Registro de Deuda" debe funcionar de manera impecable. Eso es lo que demostrará el valor real.
- **Herramientas:** La arquitectura definida en el punto anterior; el prototipo es código funcional directo.

### Testear
- Probar el sistema con compañeros, negocios locales y usuarios reales para iterar sobre la experiencia de uso.