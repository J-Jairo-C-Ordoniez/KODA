...
const catalogRepository = {
  ...
  async getPublicCatalog(tenantId: string, filters: ProductFilters = {}) {
    ...
    if (gender) {
      productFilter.gender = { in: [gender as any, 'mixto'] };
    }
    ...
    if (search && search.trim().length > 0) {
      ...
      const searchConditions: Prisma.VariantWhereInput[] = [
        ...
        { product: { is: { description: { contains: s, mode: 'insensitive' } } } },
        { product: { is: { category: { is: { name: { contains: s, mode: 'insensitive' } } } } } },
      ];
