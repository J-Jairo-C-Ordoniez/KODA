  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className={`text-xs font-black uppercase tracking-widest px-3 py-1 rounded-full ${product.isPublic ? 'bg-green-50 text-green-600' : 'bg-foreground/5 text-secondary'}`}>
      ...
      <section className="space-y-2 max-w-3xl">
        <h3 className="text-xs font-black uppercase tracking-widest text-secondary ml-1">Descripción del Producto</h3>
      ...
      <section className="space-y-8">
        ...
        {product.variants?.length === 0 ? (
           <div className="py-32 text-center space-y-4 border-2 border-dashed border-foreground/5 rounded-[60px] bg-foreground/1">
              <div className="w-24 h-24 rounded-[36px] bg-foreground/5 flex items-center justify-center mx-auto text-secondary/20">
                <Layers size={48} />
              </div>
              <p className="text-secondary font-medium">Añade tallas, colores o modelos para empezar a vender.</p>
            </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {product.variants.map((v: any) => (
              <article key={v.variantId} className="bg-background border border-foreground/5 rounded-[40px] p-8 space-y-5 hover:shadow-2xl hover:shadow-navy/10 transition-all group relative overflow-hidden flex flex-col">
                <div className="absolute -top-6 -right-6 w-24 h-24 bg-foreground/3 rounded-full group-hover:scale-110 transition-transform duration-700" />
                ...
                <div className="flex items-start justify-between relative z-10">
                  <div className="w-24 h-24 rounded-[28px] bg-foreground/5 overflow-hidden border border-foreground/5 shadow-inner shrink-0">
                    {v.images?.[0] ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-secondary/20 italic text-[10px] font-bold uppercase">N/A</div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {(() => {
                        ...
                        if (stock === 0) return <span className="px-3 py-1 bg-red-50 text-red-500 text-[10px] font-black uppercase tracking-widest rounded-full hover:bg-red-100 transition-colors">Sin Stock</span>;
                        if (stock < 5) return <span className="px-3 py-1 bg-amber-50 text-amber-600 text-[10px] font-black uppercase tracking-widest rounded-full hover:bg-amber-100 transition-colors">Stock Bajo</span>;
                        return <span className="px-3 py-1 bg-green-50 text-green-600 text-[10px] font-black uppercase tracking-widest rounded-full hover:bg-green-100 transition-colors">{stock} Unidades</span>;
                      })()}
                    </button>
                ...
                <div className="space-y-1 relative z-10">
                  ...
                  <p className="text-xs font-bold uppercase tracking-widest opacity-60 text-secondary">{v.sku || 'Sin SKU'}</p>
                </div>

                <div className="pt-4 border-t border-foreground/5 flex items-center justify-between relative z-10">
                  <div className="flex items-center gap-4">
                    <div className="space-y-0.5">
                      <p className="text-[10px] font-black uppercase tracking-widest text-secondary">Atributos</p>
                      <p className="font-bold text-primary text-xs uppercase">{v.color} • {v.size}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-black uppercase tracking-widest text-navy/40">Precio</p>
