  ...
  return (
    ...
    >
      <Button
        variant="secondary"
        onClick={prevSlide}
        className="absolute left-0 md:-left-8 z-10 w-12 h-12 rounded-full flex items-center justify-center shadow-xl border-none p-1"
      ...
      <div className="w-full relative h-130 md:h-90 bg-transparent overflow-hidden">
        {CARDS.map((card, idx) => {
      ...
      <Button
        variant="secondary"
        onClick={nextSlide}
        className="absolute right-0 md:-right-8 z-10 w-12 h-12 rounded-full flex items-center justify-center shadow-xl border-none p-1"
      ...
      >
        {CARDS.map((_, idx) => (
          <button
            ...
            className={`transition-all duration-500 rounded-full ${activeIndex === idx ? 'w-8 h-3 bg-primary' : 'w-3 h-3 bg-foreground/15 hover:bg-foreground/30'}`}
