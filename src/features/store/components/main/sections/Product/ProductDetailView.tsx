...
export default function ProductDetailView({ tenant, slug, variantId }: Props) {
    ...
    return (
      <div className="min-h-screen bg-background flex text-primary font-sans">
        ...
        <div className="flex-1 flex flex-col justify-between">
          <Header businessName={tenant.businessName} slug={slug} categoryTitle="DETALLE DE PRENDA" />
  ...
  if (error || !data || !selectedVariant) {
    return (
      <div className="min-h-screen bg-background flex text-primary font-sans">
        <SidebarLeft slug={slug} />
        <div className="flex-1 flex flex-col justify-between">
          <Header businessName={tenant.businessName} slug={slug} categoryTitle="PRODUCTO NO ENCONTRADO" />
  ...
  const displaySku = selectedVariant.sku ? selectedVariant.sku : '40041';
    ...
    <div className="min-h-screen bg-background text-primary flex font-sans selection:bg-contrast/30">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <Header
          ...
          slug={slug}
          categoryTitle={parentProduct.name}
        />
                ...
                <div className="flex items-center gap-4 overflow-x-auto pb-2">
                  {images.map((img: any, idx: number) => (
                    <button
                      ...
                      className={`
                        relative w-20 h-24 rounded-xl border overflow-hidden shrink-0 transition-all
              ...
              <div className="space-y-3 pt-4">
                {tenant.whatsApp ? (
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-3 py-5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-black text-xs uppercase tracking-[0.2em] transition-all shadow-xl shadow-emerald-500/20 active:scale-[0.98]"
                  >
                ...
                <div className="flex items-center justify-center gap-2 text-[10px] font-bold text-foreground-muted uppercase tracking-widest opacity-60 pt-2">
                  <ShieldCheck size={14} />
                  <span>Envío directo y garantizado</span>
                </div>
              </div>
            </div>
          </div>
